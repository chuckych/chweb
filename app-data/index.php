...
define('HOSTCHWEB', api_internal_base_url());
define('URLAPI', HOSTCHWEB . "/" . HOMEHOST);

clean_files('json/', 1, 'json');
clean_files('logs/', 1, 'log');
clean_files('archivos/', 1, 'xls');
clean_files('archivos/', 1, 'pdf');
clean_files('archivos/', 1, 'txt');
...
Flight::route('PUT /novedad', function () {
    ...
    $endpoint = URLAPI . "/api/v1/novedades";
    $method = 'PUT';
...
Flight::route('POST /novedad', function () {
    ...
    $endpoint = URLAPI . "/api/v1/novedades?procesar=1";

    $rs = ch_api($endpoint, [$payload], 'POST', []);
...
Flight::route('/fechas/horas', function () {
    $endpoint = URLAPI . "/api/v1/horas/dateMinMax";
...
Flight::route('/fechas/fichas', function () {
    $endpoint = URLAPI . "/api/v1/fichas/dateMinMax";
    $data = ch_api($endpoint, [], 'GET', []);
    $arrayData = json_decode($data, true);
    if (($arrayData['RESPONSE_CODE'] ?? '') == '200 OK') {
        $arrayData = $arrayData['DATA'] ?? [];
...
Flight::route('POST /get_personal_horarios', function () {
    ...
    function procesarHorarios(array $DATA)
    {
        foreach ($DATA as $legajoData) {
...
Flight::route('/horarios/asign/@legajo', function ($legajo) {
    ...
    $endpoint = URLAPI . "/api/v1/horarios/asign/legajo/$legajo";
    $data = ch_api($endpoint, [], 'GET', []);
...
Flight::route('POST /test_connect', function () {
    $endpoint = URLAPI . "/api/v1/conectar";
    $payload = Flight::request()->data;
...
Flight::route('POST /estruct/fichas/', function () {
    $endpoint = URLAPI . "/api/fichasestruct/";
    $payload = Flight::request()->data;
    $data = ch_api($endpoint, ($payload), 'POST', []);
...
Flight::route('POST /estructuras/alta/', function () {
    ...
    $endpoint = URLAPI . "/api/v1/estructuras/alta/";
    $payload = Flight::request()->data;
...
Flight::route('POST /prysmian/@tipo', function ($tipo) {
    ...
    try {
        ...
        if (!in_array($tipo, ['view', 'xls', 'pdf'])) { // Validar el tipo de reporte
        ...
        $flag = $payload['flag'] ?? microtime(true);

        switch ($payload['Reporte'] ?? '') { // Según el tipo de reporte
            case '1':
                require __DIR__ . '/php/prysmian1.php';
                break;
            case '2':
                require __DIR__ . '/php/prysmian2.php';
                break;
            case '3':
                require __DIR__ . '/php/prysmian3.php';
                break;
            default:
                Flight::json([]);
                break;
...
Flight::route('GET /liquidar/custom/campos', function () {
    try {
        ...
        Flight::json($config ?? ['separador' => ',', 'encabezados' => 0, 'campos' => [], 'filtros' => ['Lega' => [], 'Empr' => [], 'Plan' => [], 'Conv' => [], 'Sect' => [], 'Sec2' => [], 'Grup' => [], 'Sucu' => []], 'filtrosDescripcion' => ['Lega' => [], 'Empr' => [], 'Plan' => [], 'Conv' => [], 'Sect' => [], 'Sec2' => [], 'Grup' => [], 'Sucu' => []]]);
    } catch (\Throwable $th) {
...
Flight::route('POST /liquidar/custom/campos', function () {
    try {
        ...
        $filtros = array_key_exists('filtros', $payloadData) ? $payloadData['filtros'] : null;
        $filtrosDescripcion = array_key_exists('filtrosDescripcion', $payloadData) ? $payloadData['filtrosDescripcion'] : null;

        $guardados = GuardarConfiguracion($campos, $separador, $encabezados, $plantilla, $filtros, $filtrosDescripcion);
...
Flight::route('POST /liquidar/custom/plantillas', function () {
    try {
        ...
        try {
            $guardados = GuardarPlantilla($nombre, $campos, $separador, $encabezados, $slug, $filtros, $filtrosDescripcion);
        } catch (TypeError $e) {
            throw new Exception('Error al guardar la plantilla: ' . $e->getMessage(), 500);
        }
...
Flight::route('POST /personal/filtros', function () {
    ...
    $endpoint = URLAPI . "/api/v1/personal/filtros";
...
Flight::route('POST /proyectar', function () {
    ...
    $payload = $request->data ?? [];
    $arrayAuditoria = [];
    $endpoint = URLAPI . "/api/v1/proyectar";
    ...
    if ($payload['flag'] && $payload['TipoIngreso'] === '2') {
        ...
        if (file_exists($pathFile)) {
            ...
            if (is_array($legajosFromFile)) {
                ...
                echo json_encode(['status' => 'error', 'MESSAGE' => 'legajos inválidos']);
                exit;
            }

        } else {
            echo json_encode(['status' => 'error', 'MESSAGE' => 'El archivo de legajos no existe.']);
            exit;
        }
    }

    if ($payload['TipoIngreso'] === '1') {
...
Flight::route('POST /ws_procesar', function () {
    ...
    if ($payload['flag'] && $payload['procesar_por'] === '2') {
        // Construir la ruta del archivo JSON basado en el valor de 'flag'
        $pathFile = __DIR__ . '/json/' . $payload['flag'] . '_legajos_ws_procesar.json';
    ...
    if ($result) {
        ...
        if ($Legajos) {
            foreach ($Legajos as $legajo) {
                $arrayAuditoria[] = [
                    'AudTipo' => 'P',
                    'AudDato' => "Procesar Legajo: {$legajo} desde {$FechaDesde} hasta {$FechaHasta}",
                ];
            }
        }
...
Flight::route('POST|GET /custom/@file', function ($file) {
    $mapFiles = [
        ...
        'hor-per-fichas1'   => __DIR__ . '/../horas/GetPersonalFichas1.php',
        'hor-fechas'        => __DIR__ . '/../horas/GetFechasFichas1.php',
        'hor-horas'         => __DIR__ . '/../horas/GetHoras.php',
        'hor-horas-fecha'   => __DIR__ . '/../horas/GetHorasFecha.php',
        'hor-excel'         => __DIR__ . '/../horas/HorXLS.php',
        ...
        'onov-getleg'       => __DIR__ . '/../informes/inforonov/getSelect/getLegajosFichas.php',
        'onov-getemp'       => __DIR__ . '/../informes/inforonov/getSelect/getEmpFichas.php',
        'onov-getnove'      => __DIR__ . '/../informes/inforonov/getSelect/getFicNove.php',
        'onov-getgru'       => __DIR__ . '/../informes/inforonov/getSelect/getGrupFichas.php',
        'onov-getpla'       => __DIR__ . '/../informes/inforonov/getSelect/getPlanFichas.php',
        'onov-getsec2'      => __DIR__ . '/../informes/inforonov/getSelect/getSec2Fichas.php',
        'onov-getsec'       => __DIR__ . '/../informes/inforonov/getSelect/getSectFichas.php',
        'onov-getsuc'       => __DIR__ . '/../informes/inforonov/getSelect/getSucFichas.php',
        'onov-gettipoper'   => __DIR__ . '/../informes/inforonov/getSelect/getTipoPerFichas.php',
        'onov-reporte'      => __DIR__ . '/../informes/inforonov/reporte/reporteonov.php',
...
foreach (['relohabi', 'perrelo', 'identifica'] as $ruta) {
    Flight::route("GET /{$ruta}", function () use ($ruta) {
        $endpoint = URLAPI . "/api/v1/acceso/{$ruta}";

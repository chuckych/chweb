...
function estructuras(array $estructuras = [])
{
    ...
    //     "Sucu": "1,6,3,4"
    // }
    $endpoint = URLAPI . "/api/v1/estructuras";
...
{
    try {
        $endpoint = URLAPI . "/api/v1" . $endpoint;
...
{
    $endpoint = URLAPI . "/api/v1/novedades/totales/";
    $data = ch_api($endpoint, $payload, 'POST', []);
    $arrayData = json_decode($data, true);
    // print_r($payload) . exit;
    $Data = $arrayData['DATA'] ?? '';
...
{
    $endpoint = URLAPI . "/api/v1/horas/data";
...
{
    $endpoint = URLAPI . "/api/v1/novedades/data";
    $data = ch_api($endpoint, [], 'GET', ['start' => 0, 'length' => 9000]);
...
{
    $endpoint = URLAPI . "/api/_local/params";

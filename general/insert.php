...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_fichada'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['aFic']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para ingresar Fichadas.');
        echo json_encode($data);
        exit;
    };
    if(ValidaFormatoHora($_POST['RegHora'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto') ;
        echo json_encode($data);
        exit;
    }

    if (valida_campo($_POST['RegHora']) || ($_POST['RegHora']=='00:00')) {
        $data = array('status' => 'error', 'dato' => 'Campo requerido.');
        echo json_encode($data);
        exit;
    };
    if (valida_campo($_POST['datos_fichada'])) {
        $data = array('status' => 'error', 'dato' => 'datos_fichada requerido.');
        echo json_encode($data);
        exit;
    };
    ...
    if(ValidarHora($_POST['RegHora'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$_POST['RegHora']);
        echo json_encode($data);
        exit;
    }
    ...
    if(PerCierre($RegFeAs,$RegLega)){
        $data = array('status' => 'error', 'dato' => 'Fecha de Cierre es Menor o Igual: '.Fech_Format_Var($RegFeAs, ('d/m/Y')));
        echo json_encode($data);
        exit;
    }
    ...
    if (sqlsrv_num_rows($result) == 0) {
        $data = array('status' => 'error', 'dato' => 'No hay identificador asociado al Legajo.');
        echo json_encode($data);
        exit;
        sqlsrv_close($link);
    } else {
    ...
    if ($ExisteRegistro) {
        ...
        if(UpdateRegistro("UPDATE REGISTRO Set RegTipo = '$RegTipo',RegLega = '$RegLega',RegFeAs = '$RegFeAs',RegFeRe = '$RegFeRe',RegHoRe = '$RegHoRe',RegTran = '$RegTran',RegSect = '$RegSect',RegRelo = '$RegRelo',RegLect = '$RegLect',FechaHora = '$FechaHora' WHERE RegTarj = '$RegTarj' and RegFech = '$RegFech' and RegHora = '$RegHora'")) {
            ...
            if(procesar_legajo($RegLega, $RegFeAs, $RegFeAs)=='Terminado'){
                $Procesado = " - Procesado.";
                $data = array('status' => 'ok', 'dato' => $Dato2.$Procesado);
            }else{
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['baja_fichada'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['bFic']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para eliminar Fichadas.');
        echo json_encode($data);
        exit;
    };
    ...
    if(ValidarHora($RegHora)){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$RegHora);
        echo json_encode($data);
        exit;
    }

    if(PerCierre($RegFech,$RegLega)){
        $data = array('status' => 'error', 'dato' => 'Fecha de Cierre es Menor o Igual a: '.Fech_Format_Var($RegFech, ('d/m/Y')));
        echo json_encode($data);
        exit;
    }
    $RegTipo = '1';
    ...
    if ((valida_campo($_POST['Datos']))) {
        $data = array('status' => 'error', 'dato' => 'Campos requerido.');
        echo json_encode($data);
        exit;
    };
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['mod_fichada'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['mFic']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para modificar Fichadas.');
        echo json_encode($data);
        exit;
    };
    ...
    if(ValidarHora($_POST['RegHora_mod'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$_POST['RegHora_mod']);
        echo json_encode($data);
        exit;
    }
    ...
    if(PerCierre($RegFech1,$RegLega)){
        $data = array('status' => 'error', 'dato' => 'Fecha de Cierre es Menor o Igual a: '.Fech_Format_Var($RegFech1, ('d/m/Y')));
        echo json_encode($data);
        exit;
    }
    ...
    if ($ExisteRegistro) {
        ...
        $data = array('status' => 'error', 'dato' => 'No existe Fichada');
        echo json_encode($data);
        exit;
    }
}
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_novedad'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['aNov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para agregar Novedades.');
        echo json_encode($data);
        exit;
    };
    ...
    if(ValidarHora($_POST['FicHoras'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$_POST['FicHoras']);
        echo json_encode($data);
        exit;
    }
    ...
    if ((valida_campo($_POST['FicHoras'])) || (valida_campo($_POST['FicNove']))) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>Horas y Novedad</strong> son requeridos!');
        echo json_encode($data);
        exit;
    };
    if (valida_campo($_POST['datos_novedad'])) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>datos_novedad</strong> requerido!');
        echo json_encode($data);
        exit;
    };
    ...
    if(PerCierre($FicFech,$FicLega)){
        $data = array('status' => 'error', 'dato' => 'Fecha de Cierre es Menor o Igual a: '.Fech_Format_Var($FicFech, ('d/m/Y')));
        echo json_encode($data);
        exit;
    } 
    ...
    if (!$DiaLaboral && $FicCate == 0) {
        // echo $DiaLaboral;
        $data = array('status' => 'Error', 'dato' => 'Error. Solo se permiten Novedades Secundarias en día no Laboral');
        echo json_encode($data);
        exit;
    ...
    /** Chequeamos si existe una novedad del mismo tipo en Fichas3 *//** Si existe la borramos */
        ...
        if(InsertRegistro("INSERT INTO FICHAS3 (FicLega,FicFech,FicTurn,FicNove,FicNoTi,FicHoras,FicJust,FicObse,FicCaus,FicEsta,FicCate,FicComp,FechaHora) Values('$FicLega','$FicFech',1,'$FicNove','$NovTipo','$FicHoras','$FicJust','$FicObse','$FicCaus',1,'$FicCate','00:00','$FechaHora')")){
            audito_ch('A', $Dato); /** Grabamos en Auditor */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_novedad'] == 'Mod')) {
    ...
    if ($_SESSION["ABM_ROL"]['mNov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para modificar Novedades.');
        echo json_encode($data);
        exit;
    };
    ...
    $MNove     = explode('-', $_POST['CNove']); /** Cod Nov, Tipo y Categoria */
    ...
    $Dato2 = 'Nov: ('.$NovCodi.') '.$NovDesc;
        ...
        if(UpdateRegistro("UPDATE FICHAS3 SET FicNove = '$FicNove', FicNoTi='$NovTipo', FicHoras='$FicHoras', FicJust='$FicJust', FicObse='$FicObse', FicCaus='$FicCaus', FicCate='$FicCate', FechaHora = '$FechaHora' WHERE FicNove = '$MNoveCod' AND FicLega = '$FicLega' AND FicFech = '$FicFech' AND FicTurn='1'")){
            audito_ch('M', $Dato); /** Grabamos en Auditor */
            if(procesar_legajo($FicLega, $FicFech, $FicFech)=='Terminado'){
                $Procesado = " - Procesado.";
                $data = array('status' => 'ok', 'dato' => $Dato2.$Procesado, 'tipo' => 'mod');
            }else{
                $Procesado = " - Sin procesar.";
                $data = array('status' => 'ok', 'dato' => $Dato2.$Procesado);
            }
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['baja_novedad'] == 'true')) {
    if ($_SESSION["ABM_ROL"]['bNov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para eliminar Novedades.');
        echo json_encode($data);
        exit;
    };

    $datos = explode('-', $_POST['Datos']); /** FicNov, FicFech, FicLega */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_horas'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['aHor']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para agregar Horas.');
        echo json_encode($data);
        exit;
    };
    ...
    if(ValidarHora($_POST['Fic1HsAu2'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$_POST['Fic1HsAu2']);
        echo json_encode($data);
        exit;
    }

    if ((valida_campo($_POST['Fic1Hora'])) || (valida_campo($_POST['Fic1HsAu2']))) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>Tipo Hora y Autorizadas</strong> son requeridos!');
        echo json_encode($data);
        exit;
    };
    ...
    if (!$Ausente) {
        $data = array('status' => 'Error', 'dato' => 'Error. No se permite la carga de Horas en ausente');
        echo json_encode($data);
        exit;
    }
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['baja_Hora'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['bHor']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para eliminar Horas.');
        echo json_encode($data);
        exit;
    };

    $datos    = explode('-', $_POST['Datos']); /** FicHora, FicFech, FicLega */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_horas'] == 'mod')) {  
    ...
    if ($_SESSION["ABM_ROL"]['mHor']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para modificar Horas.');
        echo json_encode($data);
        exit;
    };
    ...
    if (!$ExisteRegistro) {
        $data = array('status' => 'Error', 'dato' => 'Error. No existe el registro');
        echo json_encode($data);
        exit;
    }
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_OtrasNov'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['aONov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para agregar Otras Novedades.');
        echo json_encode($data);
        exit;
    };
    ...
    if ((valida_campo($_POST['FicONov'])) || (valida_campo($_POST['FicValor']))) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>Novedad y Valor</strong> son requeridos!');
        echo json_encode($data);
        exit;
    };
    if (valida_campo($_POST['datos_OtrasNov'])) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>datos_OtrasNov</strong> requerido!');
        echo json_encode($data);
        exit;
    };
    ...
    if ($ExisteRegistro) {
        $data = array('status' => 'Error', 'dato' => 'Error. Ya existe el registro');
        echo json_encode($data);
        exit;
    }
    require __DIR__ . '../../config/conect_mssql.php';
    ...
    $Dato2 = 'Otra Novedad: ('.$ONovCodi.') '.$ONovDesc;
        ...
        if(InsertRegistro("INSERT INTO FICHAS2 (FicLega,FicFech,FicTurn,FicONov,FicValor,FicObsN,FechaHora) VALUES ('$FicLega','$FicFech',1,'$FicONov', '$FicValor','$FicObsN','$FechaHora')")){
            $data = array('status' => 'ok', 'dato' => $Dato2, 'tipo'=>'');
            audito_ch('A', $Dato); /** Grabamos en Auditor */
            echo json_encode($data);
            exit;
            sqlsrv_close($link);
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['baja_ONov'] == 'true')) {
    ...
    if ($_SESSION["ABM_ROL"]['bONov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para eliminar Otras Novedades.');
        echo json_encode($data);
        exit;
    };

    $datos    = explode('-', $_POST['Datos']); /** FicOnov, FicFech, FicLega */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_OtrasNov'] == 'mod')) {
    ...
    if ($_SESSION["ABM_ROL"]['mONov']=='0'){
        $data = array('status' => 'error', 'dato' => 'No tiene permiso para modificar Otras Novedades.');
        echo json_encode($data);
        exit;
    };
    ...
    if (!$ExisteRegistro) {
        $data = array('status' => 'Error', 'dato' => 'No existe el registro');
        echo json_encode($data);
        exit;
    }
    ...
    if ($ExisteRegistro) {
        $data = array('status' => 'Error', 'dato' => 'Ya existe el registro');
        echo json_encode($data);
        exit;
    }
    require __DIR__ . '../../config/conect_mssql.php';
    ...
    $Dato2 = 'Otra Novedad: ('.$ONovCodi.') '.$ONovDesc;
        ...
        if(UpdateRegistro("UPDATE FICHAS2 Set FicValor = '$FicValor', FicObsN = '$FicObsN', FechaHora = '$FechaHora' WHERE FicLega = '$FicLega' and FicFech = '$FicFech' and FicTurn = 1 and FicONov = '$FicONov'")){
            $data = array('status' => 'ok', 'dato' => $Dato2, 'tipo'=>'mod');
            audito_ch('M', $Dato); /** Grabamos en Auditor */
            echo json_encode($data);
            exit;
            sqlsrv_close($link);
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_Citación'] == 'true')) {
    ...
    if ((($CitEntra=='00:00') && ($CitSale=='00:00'))) {
        $data = array('status' => 'error', 'dato' => 'Campos Entrada y Salida no pueden estar en 00:00');
        echo json_encode($data);
        exit;
    };
    if ((($CitEntra=='00:00'))) {
        $data = array('status' => 'error', 'dato' => 'Campo Entrada no puede estar en 00:00');
        echo json_encode($data);
        exit;
    };
    if ((($CitSale=='00:00'))) {
        $data = array('status' => 'error', 'dato' => 'Campo Salida no puede estar en 00:00');
        echo json_encode($data);
        exit;
    };
    if(ValidarHora($_POST['CitEntra'])){
        $data = array('status' => 'error', 'dato' => 'Formato de Hora incorrecto: '.$_POST['CitSale']);
        echo json_encode($data);
        exit;
    }
    ...
    if ((HoraMin($CitEntra) >= HoraMin($CitSale))) {
        $data = array('status' => 'error', 'dato' => 'El Horario de entrada no puede ser Mayor o Igual de Salida.');
        echo json_encode($data);
        exit;
    };
    ...
    if ((HoraMin($CitDesc) >= ($Intervalo))) {
        $data = array('status' => 'error', 'dato' => 'El Descanso no puede ser Mayor al intervalo entre Entrada y Salida<br/>Intervalo: '.FormatHora($Intervalo).'<br/>Descanso: '.$CitDesc);
        echo json_encode($data);
        exit;
    };

    if (valida_campo($_POST['datos_Citación'])) {
        $data = array('status' => 'error', 'dato' => 'Campo <strong>datos_Citación</strong> requerido!');
        echo json_encode($data);
        exit;
    };
    ...
    if ($ExisteCitacion) {
        ...
        if(InsertRegistro("INSERT INTO CITACION (CitLega,CitFech,CitTurn,CitEntra,CitSale,CitDesc,FechaHora) VALUES ( '$FicLega','$FicFech','1','$CitEntra','$CitSale','$CitDesc','$FechaHora' )")){
            ...
            if(procesar_legajo($FicLega, $FicFech, $FicFech)=='Terminado'){
                $Procesado = " - Procesado.";
                $data = array('status' => 'ok', 'dato' => $Dato2.$Procesado, 'tipo' => 'alta');
            }else{
                $Procesado = " - Sin procesar.";
                $data = array('status' => 'ok', 'dato' => $Dato2.$Procesado, 'tipo' => 'alta');
            }

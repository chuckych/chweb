...
$where_condition = $sqlTot = $sqlRec = "";
 ...
 $sql_query="SELECT PERSONAL.LegNume AS 'pers_legajo', PERSONAL.LegApNo AS 'pers_nombre' FROM FICHAS2 INNER JOIN PERSONAL ON FICHAS2.FicLega=PERSONAL.LegNume INNER JOIN FICHAS ON FICHAS2.FicLega=FICHAS.FicLega WHERE FICHAS2.FicFech BETWEEN '$FechaIni' AND '$FechaFin' $FilterEstruct $FilterEstruct2 $FiltrosFichas GROUP BY PERSONAL.LegNume, PERSONAL.LegApNo";
...
if (!empty($Per2)) {
    if (!CountRegistrosMayorCero("SELECT FICHAS.FicLega FROM FICHAS WHERE FICHAS.FicLega = '$Per3'")) {
        $data[] = array(

...
$(function () {
    ...
    function Select2Estruct(selector, multiple, placeholder, estruct, url, parent) {
        ...
        $(selector).select2({
            ...
            // almacenar en localStorage con la funcion ls.set()
            let selectId = $(this).attr('id');
            let selectedData = $(this).select2('data');
            localStorage.setItem(LS_SELECT + "_" + selectId, JSON.stringify(selectedData));
    ...
    function LoadSelects() {
        ...
        selects.forEach(function (selector) {
            ...
            let storedData = localStorage.getItem(LS_SELECT + "_" + selectId);
            console.log(storedData);
            if (storedData) {
                let parsedData = JSON.parse(storedData);
                $(selector).select2('data', parsedData);
            }
        });
    }

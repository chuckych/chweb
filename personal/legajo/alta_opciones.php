...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_nacion'] == 'true')) {
    ...
    $NacDesc  = (test_input(($_POST['NacDesc']))); /** Descripcion */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_provincia'] == 'alta_provincia')) {
    ...
    $desc_prov  = (test_input(($_POST['desc_prov']))); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta_localidad'] == 'alta_localidad')) {
    ...
    $desc_local  = test_input($_POST['desc_local']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
    // print_r($desc_local);
    // exit;
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_planta')) {
    ...
    $desc_planta  = test_input($_POST['desc_planta']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
    ...
    if (sqlsrv_num_rows($result) > 0) {
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_PLANTASInsert @PlaCodi=?,@PlaDesc=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_empresa')) {
    ...
    $options = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
    $data    = array();
    $EmpRazon     = test_input($_POST['EmpRazon']); /** Razon Social */
    $EmpTipo      = test_input($_POST['EmpTipo']); /** Empresa Tipo */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_sector')) {
    ...
    $desc_sector  = test_input($_POST['desc_sector']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
    ...
    if (sqlsrv_num_rows($result) > 0) {
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_SECTORESInsert @SecCodi=?,@SecDesc=?,@SecTaIn=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_seccion')) {
    ...
    $Se2Desc   = test_input($_POST['Se2Desc']); /** Descripcion */
    ...
    $result  = sqlsrv_query($link, $query, $params, $options);
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_SECCIONInsert @SecCodi=?,@Se2Codi=?,@Se2Desc=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_grupo')) {
    ...
    $desc_grupo  = test_input($_POST['desc_grupo']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
    ...
    if (sqlsrv_num_rows($result) > 0) {
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_GRUPOSInsert @GruCodi=?,@GruDesc=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_sucur')) {
    ...
    $desc_sucur  = test_input($_POST['desc_sucur']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
    ...
    if (sqlsrv_num_rows($result) > 0) {
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_SUCURSALESInsert @SucCodi=?,@SucDesc=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_tarea')) {
    ...
    $desc_tarea  = test_input($_POST['desc_tarea']); /** Descripcion */
    $FechaHora = date('Ymd H:i:s');
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato_conv'] == 'alta_convenio')) {
    ...
    $desc_convenio = test_input($_POST['desc_convenio']); /** Descripcion */
    $ConDias       = test_input($_POST['ConDias']); /** 1 día cada 'ConDias' trabajados */
    $ConTDias      = test_input($_POST['ConTDias']); /** Dias Tipo */
    $FechaHora     = date('Ymd H:i:s');
    ...
    if (sqlsrv_num_rows($result) > 0) {
        ...
        while ($fila = sqlsrv_fetch_array($result)) {
            ...
            $sql = "exec DATA_CONVENIOInsert @ConCodi=?,@ConDesc=?,@ConDias=?,@ConTDias=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato_conv'] == 'mod_convenio')) {
    ...
    $desc_convenio = test_input($_POST['desc_convenio']); /** Descripcion */
    $codConv       = test_input($_POST['codConv']); /** 1 día cada 'ConDias' trabajados */
    $ConDias       = test_input($_POST['ConDias']); /** 1 día cada 'ConDias' trabajados */
    $ConTDias      = test_input($_POST['ConTDias']); /** Dias Tipo */
    $FechaHora     = date('Ymd H:i:s');
    ...
    /** fin */
            ...
            $sql = "exec DATA_CONVENIOUpdate @ConCodi=?,@ConDesc=?,@ConDias=?,@ConTDias=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta-diasvac'] == 'alta-diasvac')) {
    ...
    $CVConv    = test_input($_POST['cod-diasvac']); /** cod-diasvac */
    $CVAnios   = test_input($_POST['anios']); /** Años */
    $CVMeses   = test_input($_POST['meses']); /** Meses */
    $CVDias    = test_input($_POST['diasvac']); /** Diasvac */
    $FechaHora = date('Ymd H:i:s');
    ...
    $Dato    = 'Antiguedad Convenio: '. $CVConv . ': ' . $CVAnios .'-'.$CVMeses.'-'.$CVDias;
        ...
        /** fin */            
            ...
            $sql = "exec DATA_CONVVACAInsert @CVConv=?,@CVAnios=?,@CVMeses=?,@CVDias=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['del_ConvVac'] == 'true')) {
    ...
    $CVConv    = test_input($_POST['del_cod']); /** cod-diasvac */
    $CVAnios   = test_input($_POST['del_anios']); /** Años */
    $CVMeses   = test_input($_POST['del_meses']); /** Meses */
    $CVDias    = test_input($_POST['del_dias']); /** Meses */
    $FechaHora = date('Ymd H:i:s');
            ...
            $sql = "exec DATA_CONVVACADelete @CVConv=?,@CVAnios=?,@CVMeses=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['alta-feriConv'] == 'alta-feriConv')) {
    ...
    $Dato    = 'Feriado Convenio: '. $CFConv . '. Fecha: ' . Fech_Format_Var($CFFech,'d/m/Y') .'. Desc: '.$CFDesc. ' CFInFeTR = '.$CFInFeTR;
        ...
        /** fin */
            ...
            $sql = "exec DATA_CONVFERIInsert @CFConv=?,@CFFech=?,@CFDesc=?,@CFCodM=?,@CFCodJ=?,@CFInfM=?,@CFInfJ=?,@CFCodM2=?,@CFCodJ2=?,@CFCodM3=?,@CFCodJ3=?,@CFInFeTR=?,@CFInMeNL=?,@CFInJoNL=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['del_ConvFeri'] == 'true')) {
    ...
    $FechaHora = date('Ymd H:i:s');
        ...
        $Dato    = 'Feriado Convenio: '. $CFConv . '. Fecha: ' . Fech_Format_Var($CFFech,'d/m/Y') .'. Desc: '.$CFDesc;
            ...
            $sql = "exec DATA_CONVFERIDelete @CFConv=?,@CFFech=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['dato'] == 'alta_perineg')) {
    ...
    $Dato2    = 'Leg: '. $InEgLega . '.<br/> In: ' . Fech_Format_Var($InEgFeIn,'d/m/Y') .'. Eg: '.Fech_Format_Var($InEgFeEg,'d/m/Y');
        ...
        /** fin */
            ...
            $sql = "exec DATA_PERINEGInsert @InEgLega=?,@InEgFeIn=?,@InEgFeEg=?,@InEgCaus=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelPerineg'] == 'true')) {
    ...
    $Dato    = 'Leg: '. $InEgLega . '. In: ' . Fech_Format_Var($InEgFeIn,'d/m/Y');
            ...
            $sql = "exec DATA_PERINEGDelete @InEgLega=?,@InEgFeIn=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['PERPREMI'] == 'PERPREMI')) {
    ...
    $LPreLega    = test_input($_POST['LPreLega']); /** Legajo */
    $LPreCodi   = test_input($_POST['LPreCodi']); /** Codigo premio */
    $FechaHora = date('Ymd H:i:s');
    ...
    $Dato    = 'Premio: ('.$LPreCodi.'). Legajo: '. $LPreLega;
        ...
        /** fin */            
            ...
            $sql = "exec DATA_PERPREMIInsert @LPreLega=?,@LPreCodi=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelPerPremi'] == 'true')) {
    ...
    $Dato    = 'Premio: ('.$LPreCodi.'). Legajo: '. $LPreLega;
            ...
            $sql = "exec DATA_PERPREMIDelete @LPreLega=?,@LPreCodi=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['OTROCONLEG'] == 'OTROCONLEG')) {
    ...
    $OTROConLega  = test_input($_POST['OTROConLega']); /** Legajo */
    ...
    $Dato    = 'Concepto: ('.$OTROConCodi.'). Legajo: '. $OTROConLega;
        ...
        /** fin */            
            ...
            $sql = "exec DATA_OTROCONLEGInsert @OTROConLega=?,@OTROConCodi=?, @OTROConValor=?, @FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelOtroConLeg'] == 'true')) {
    ...
    $Dato    = 'Concepto: ('.$OTROConCodi.'). Legajo: '. $OTROConLega;
            ...
            $sql = "exec DATA_OTROCONLEGDelete @OTROConLega=?,@OTROConCodi=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['PERHOAL'] == 'PERHOAL')) {
    ...
    $LegHoAl  = test_input($_POST['LegHoAl']); /** Legajo */
    ...
    $Dato    = 'Horario Alternativo: ('.$LegHoAl.'). Legajo: '. $LeHALega;
        ...
        /** fin */            
            ...
            $sql = "exec DATA_PERHOALTInsert @LeHALega=?,@LeHAHora=?, @FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelPerHoAl'] == 'true')) {
    ...
    $Dato    = 'Horario Alternativo: ('.$LeHAHora.'). Legajo: '. $LeHALega;
            ...
            $sql = "exec DATA_PERHOALTDelete @LeHALega=?,@LeHAHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['IDENTIFICA'] == 'IDENTIFICA')) {
    ...
    $Dato    = 'Identificador: ('.$IDCodigo.'). Legajo: '. $IDLegajo;
        ...
        /** Query revisar si existe un registro igual */
            ...
            $IDComedor='';$IDInvitado='0';$IDAcceso='1';$IDAlerta='0';$IDProvis='0';$IDAsigna='1';$IDTDoc='0';$IDDocu='0';$IDDisp='';$IDPrivi='0';$IDPass='';$IDFing='0';$IDFing1='0';$IDFing2='0';$IDFing3='0';$IDFing4='0';$IDFing5='0';$IDFing6='0';$IDFing7='0';$IDFing8='0';$IDFing9='0';$IDCap01=$IDCap01;$IDCap02=$IDCap02;$IDCap03=$IDCap03;$IDCap04=$IDCap04;$IDCap05=$IDCap05;$IDCap06=$IDCap06;$IDSupTarj='';$IDSupAdmLev='0';$IDSupAutMod='0';$IDSupName='';$IDSupPass='';$IDSupFinger1='0';$IDSupDuress1='0';$IDSupFinger2='0';$IDSupDuress2='0';$IDSupStart='17530101';$IDSupExpiry='17530101';$IDSupTarjFC='0';
        /** fin */            
            // $procedure_params = array(
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelIdentifica'] == 'true')) {
    ...
    $Dato    = 'Identificador: ('.$IDCodigo.'). Legajo: '. $IDLegajo;
            ...
            $sql = "exec DATA_IDENTIFICADelete @IDCodigo=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['GrupoHabi'] == 'true')) {
    ...
    $Dato    = 'Grupo Capt: ('.$LegGrHa.'). Legajo: '. $LegajoGrHa;
            ...
            $sql = "UPDATE PERSONAL SET PERSONAL.LegGrHa = '$LegGrHa' WHERE PERSONAL.LegNume = '$LegajoGrHa'"; /** Query */
            // print_r($sql);exit;

            $stmt = sqlsrv_prepare($link, $sql); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['PERRELO'] == 'true')) {
    ...
    $Dato    = 'Reloj habilitado: ('.$RelRelo.'), Marca: ('.$RelReMa.'). Legajo: '. $RelLega;
        ...
        /** fin */            
            ...
            $sql = "exec DATA_PERRELOInsert @RelLega=?,@RelReMa=?,@RelRelo=?,@RelFech=?,@RelFech2=?,@FechaHora=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && ($_POST['DelPerrelo'] == 'true')) {
    ...
    $Dato    = 'Reloj habilitado: ('.$RelRelo.'), Marca: ('.$RelReMa.'). Legajo: '. $RelLega;
            ...
            $sql = "exec DATA_PERRELODelete @RelLega=?, @RelReMa=?, @RelRelo=?"; /** Query del Store Prcedure */
            $stmt = sqlsrv_prepare($link, $sql, $procedure_params); /** preparar la sentencia */

...
$url = "http://awsapi.chweb.ar:7575/attention/api/punch-event/light/" . $flags_lastDate; // SIN FOTO
...
if (empty($array['payload'])) {
    $flags = (getDataIni($pathFlags));
    $flags_lastDate = $flags['flags']['lastDate'];
    statusFlags(1, $pathFlags, $flags_lastDate);
    Flight::json("No Hay Registros Nuevos");
    exit;
}

$columncreatedDate = (array_column($array['payload'], 'createdDate'));
$columnemployeId = (array_column($array['payload'], 'employeId'));
$columnIdPunch = (array_column($array['payload'], 'id'));
$columnIdCompany = (array_column($array['payload'], 'companyCode'));
$columnAppVersion = (array_column($array['payload'], 'appVersion'));
$columnLocked = (array_column($array['payload'], 'locked'));
$ultimacreatedDate = (end($columncreatedDate));

foreach ($columnAppVersion as $key => $apv) {
    $colApp[] = sanitizeAppVersion($apv);
}

$t = count($columnIdPunch);
$DataRequest = array(
    'Company' => CountAgrupArray($columnIdCompany, 'company'),
...
foreach ($columnIdPunch as $keyPunch => $idPunch) {
    ...
    try {
        ...
        $array2 = json_decode(getEvents($url2), true);

        $array2['type'] = $array2['type'] ?? '';
        $array2['status'] = $array2['status'] ?? '';
        $array2['message'] = $array2['message'] ?? '';
        // $array2['payload'] = $array2['payload'] ?? '';

        if ($array2['status'] == 500) {
            $fechaHora = date('Y-m-d H:i:s');
            $texterr = "$fechaHora Error: \"$array2[error]\" Path: \"$array2[path]\"";
            sendEmailTask("Task Mobile: $array2[error]", "<pre>$array2[timestamp]<br>$array2[path]<pre>");
            // exit;
        } else {
            if ($array2['payload']) {
...
// exit;
    ...
    if (!empty($arrayData)) {
        ...
        foreach ($arrGroup as $key => $valor) {
            ...
            if ($idZone != '0') { // Si la Zona es diferente a 0 entonces se calcula el evento consultando la tabla de zona y evento
                ...
                $eventZone = $a['evento'] ?? '0'; // Evento de la zona
            }
            ...
            if (intval($employeId) > 0) {
                ...
                $insertDB = pdoQuery($query);
            }

            if ($insertDB) { // Si se guarda correctamente insertanmos en la tabla fichadas de 
        ...
        //statusFlags(1, $pathFlags, $first_element['createdDate']); // 
    } else {
        // $flags = (getDataIni($pathFlags));
        // $flags_lastDate = $flags['flags']['lastDate'];
    ...
    unset($insertCH);
    unset($insertCH_Fail);
    // fileLog("Fin de keyPunch : $keyPunch", __DIR__ . '/logs/' . date('Ymd') . 'Bucle_keyPunch.log');
}

// file_put_contents(__DIR__ . '/logs/' . date('Ymd') . 'Bucle_keyPunch.log', '');

fileLogJson($ultimacreatedDate, 'createdDate.json', false); // un json con la fecha de la ultima descarga
statusFlags(1, $pathFlags, $ultimacreatedDate);
$end = microtime(true);
$time = round($end - $start, 2);
$r = json_encode(array('detalle' => $DataRequest, 'time' => ($time)), JSON_PRETTY_PRINT);
fileLog($r, __DIR__ . '/logs/' . date('Ymd') . '_aws_task_light.log');

Flight::json(array('detalle' => $DataRequest, 'time' => ($time)));

...
function response($data, $total, $msg = 'OK', $code = 200, $tiempoScript = 0, $count = 0, $idCompany)
{
    ...
    $pathLog  = __DIR__ . '../../../logs/' . date('Ymd') . '_log_AddDevice' . $idCompany . '.log'; // path Log Api
    /** start text log*/
...
if ($insert) {
    $a = simple_pdoQuery("SELECT * FROM `reg_device_` WHERE `phoneid` = '$devicePhoneID' AND `id_company` = '$idCompany'");
    $MESSAGE = 'OK';
    $arrayData = array(
        'id'            => $a['id'],

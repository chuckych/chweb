...
function idPunchEvent()
{
    ...
    if ($idPunchEvent) {
        foreach ($idPunchEvent as $key => $value) {
            $i[] = intval($value);
        }
        return ($i);
    }
...
if (pdoQuery($deleteEnroll)) {
    $sendApi = sendApiMobileHRP(json_encode($payload), $urlAppMobile, "attention/api/access/enroll-users", $idCompany, true);
...
$tiempoScript = round($finScript - $iniScript, 2);
$countData    = 0;
(response($responseApi->message, $countData, $MESSAGE, 'OK', $tiempoScript, $countData, $idCompany));
exit;

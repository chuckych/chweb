...
ini_set('display_errors', '0');
    ...
    $Empresas         = (!empty($data_emp)) ? "AND PERSONAL.LegEmpr IN ($data_emp) ":'';
    $EmpresasFichas   = (!empty($data_emp)) ? "AND FICHAS.FicEmpr IN ($data_emp) ":'';
    // $data_pla      = (estructura_rol('GetEstructRol', $_GET['_r'], 'plantas', 'planta'));
    $data_pla         = $_SESSION['PlanRol'];
    $Plantas          = (!empty($data_pla)) ? "AND PERSONAL.LegPlan IN ($data_pla) ":'';
    $PlantasFichas    = (!empty($data_pla)) ? "AND FICHAS.FicPlan IN ($data_pla) ":'';
    // $data_con      = (estructura_rol('GetEstructRol', $_GET['_r'], 'convenios', 'convenio'));
    $data_con         = $_SESSION['ConvRol'];
    $Convenios        = (!empty($data_con)) ? "AND PERSONAL.LegConv IN ($data_con) ":'';
    $ConveniosFichas  = (!empty($data_con)) ? "AND FICHAS.FicConv IN ($data_con) ":'';
    // $data_sec      = (estructura_rol('GetEstructRol', $_GET['_r'], 'sectores', 'sector'));
    $data_sec         = $_SESSION['SectRol'];
    $Sectores         = (!empty($data_sec)) ? "AND PERSONAL.LegSect IN ($data_sec) ":'';
    $SectoresFichas   = (!empty($data_sec)) ? "AND FICHAS.FicSect IN ($data_sec) ":'';

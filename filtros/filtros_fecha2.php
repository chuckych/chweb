...
if (($_datos == 'general') || ($_datos == 'mishoras')) {
    $_range = (($_GET['_range'] ?? '') === 'on') ? true : false;
    $col = 'col-sm-6';
...
?>
    <div class="col-12 col-sm-6 mb-2 mb-sm-0">
        <?php if (($_datos != 'mishoras')) { ?>
            <button type="button" class="mt-2 <?= $OcultarCount ?> float-left btn btn-sm mr-1 opa9 fontq w100 btn-custom" data-toggle="modal" data-target="#Total_pdia" id="Total_dia">
                Total Día
            </button>
        <?php } ?>
        <button type="button" class="mt-2 d-none d-sm-block btn btn-sm opa9 fontq w100 btn-custom" data-toggle="modal" data-target="#Total_General">
            Totales
        </button>
    </div>
<?php } else {
...
<div class="col-12 mb-2 mb-sm-0 <?= $col ?>">
    <div class="d-flex justify-content-sm-end justify-content-center">
        <input type="text" readonly class="mx-2 form-control text-center w250 mt-2 mt-sm-0 ls2" name="_dr" id="_dr" placeholder="<?= fechformat($FechaIni) . ' - ' . fechformat($FechaFin) ?>">
        <!-- <button type="submit" id="btnFiltrar" class="d-none d-sm-block btn h40 btn-info px-4 border-0 fontq float-right mt-2 mt-sm-0 btn-custom">Filtrar</button> -->
...
if (($_datos != 'cta_horas') && ($_datos != 'mobile')) {
    ...
    if (!$array[0]['error']) {
        ...
        foreach ($data2 as $key => $value) {
            ...
            $dia = '<small><div class="d-flex align-items-center align-middle text-white radius h40 px-4 border fw4 opa9 text-center bg-custom">' . $dia .= '</div></small>';
        }
    }
}

if (($_datos != 'cta_horas') && ($_datos != 'mobile') && ($_datos != 'mishoras')) {
    // if (!$array[0]['error']) {
    $si_datos = '1';
    /** para mostrar el botón de totales en modulo general */
    // $OcultarCount = ($_datos == 'mishoras') ? 'd-none': '';
...
?>
    ...
    <div class="col-6 d-none d-sm-block pt-2">

// si el documento a cargado completamente
document.addEventListener('DOMContentLoaded', function () {
    ...
    const select2Estruct = (selector, multiple, placeholder, estruct) => {
        ...
        const url = DIR_APP_DATA + "/personal/filtros";
        $(selector).select2({
            multiple: multiple,
            allowClear: true,
            language: "es",
    ...
    getFechas().then(data => {
        ...
        $('.selectjs_reporte').on('select2:select', function (e) {
            ...
            const rowFiltros3 = document.querySelector('#row_filtros_3') ?? null;
            rowFiltros3.hidden = true;
            btnConfigAct.hidden = true;
            div_table_tipo_hora.hidden = true;
            btnPdf.hidden = true;

            if (value == 2) {

                btnConfigAct.hidden = false;
                div_table_tipo_hora.hidden = false;
                btnPdf.hidden = true;
    ...
    const getData = (action) => {
        ...
        const accionesDisponibles = ['view', 'xls', 'pdf'];
    ...
    const submitData = async (action) => {
        ...
        axios.post(DIR_APP_DATA + '/prysmian/' + action, {
            ...
            NovA: [1],
            LegTipo: [LegTipo],
            Lega: reporte == '3' ? $("#selectjs_personal").val() : [],
            Sect: reporte == '3' ? $("#selectjs_sector").val() : [],
            start: 0,
            length: 10000,
            flag: FLAG,
            data: { ...Params, ...Values },
            ...
            if (data.data) {
                ...
                switch (reporte) {
                    ...
                    case '2':
                        ...
                        break;
                    case '3':
                        titleTabla = 'Reporte de Liquidación';
                        if (tipo === 'xls') { break; } // en excel no se muestra la tabla
                        if (tipo === 'pdf') { break; } // en excel no se muestra la tabla
                        datos = data.data.data;
                        const columnKeysHoras = Object.keys(datos[0] ?? {});
                        const keysInvalid = ['EstFic', 'EstUltFic'];
                        const keysLeft = ['Apellido y Nombre', 'Día', 'Fecha', 'Sector'];
                        // eliminar las keys invalidas del columnKeysHoras
                        columnKeysHoras.forEach((key, index) => {
                            if (keysInvalid.includes(key)) {
                                columnKeysHoras.splice(index, 1);
                            }
                        });
                        let index3 = 0;

                        for (const key of columnKeysHoras) {
                            index3++;
                            columnConfigs.push({
                                data: '',
                                title: key,
                                className: keysLeft.includes(key) ? '' : 'text-center', // Verificar si la key está en el array
                                render: (data, type, row, meta) => {
                                    if (key === 'Fecha') {
                                        return moment(row[key], 'YYYY-MM-DD').format('DD/MM/YYYY');
                                    }
                                    if (key === 'Ingreso' || key === 'Egreso') {
                                    }
                                    return row[key];
                                }
                            });
    ...
    const dt_grilla = async (idTable, dataSource, columnConfigs, selectorDiv, titulo = '', opt = {}) => {
        ...
        if (dataSource.length < 1) {
            notify('No hay resultados.', 'info', 2000, 'right');

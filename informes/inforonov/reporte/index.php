<?php
error_log("INFO: Ingreso a reporteonov.php", 3, "inforonov/reporte/reporteonov.log");
...
if (($_SERVER["REQUEST_METHOD"] == "POST")) {
    ...
    $_nombre = FusNuloPOST('_nombre', "InforONov"); /**  Nombre del archivo*/
    $_titulo = FusNuloPOST('_titulo', ""); /**  titulo del reporte*/
    $_Por = FusNuloPOST("_Por", 'Leg'); /** Agrupar por: Fech,Lega, ApNo */


    $_titulo = 'INFORME DE OTRAS NOVEDADES';
    $_nombre = $_nombre == '' ? strtoupper($_titulo) : $_nombre;
    $title = ($_nombre == 'InforONov') ? strtoupper($_titulo) : $_nombre;
    ...
    try {
        ...
        file_put_contents("../../../logs/error/Error_PDF_InforONov_" . date('Ymd') . ".log", $e->getMessage(), FILE_APPEND | LOCK_EX);
        exit();
    }
}
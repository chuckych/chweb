...
FROM $ColData
    ...
    AND $ColData.FicFech = FICHAS.FicFech
    AND $ColData.FicTurn = FICHAS.FicTurn
    INNER JOIN $Col ON $id = $DescCodi
    INNER JOIN PERSONAL ON FICHAS.FicLega = PERSONAL.LegNume
WHERE $ColData.FicFech BETWEEN '$FechaIni' AND '$FechaFin'
    AND $id > 0 $FiltroQ $Calculos $FilterEstruct $FiltrosFichas
GROUP BY $id,
    $Desc
ORDER BY $Desc";
// print_r($query); exit;

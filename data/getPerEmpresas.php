...
$q = $_GET['q'];
    ...
    $query = "SELECT PERSONAL.LegEmpr as 'id',
        EMPRESAS.EmpRazon as 'text'
    FROM PERSONAL
        INNER JOIN EMPRESAS ON PERSONAL.LegEmpr = EMPRESAS.EmpCodi
    WHERE PERSONAL.LegNume > '0'
        AND PERSONAL.LegFeEg = '17530101'
        AND CONCAT(PERSONAL.LegEmpr, EMPRESAS.EmpRazon) LIKE '%$q%'
        AND PERSONAL.LegEmpr > '0'
        AND PERSONAL.LegFeEg = '17530101' $FilterEstruct $filtros
    GROUP BY PERSONAL.LegEmpr,
        EMPRESAS.EmpRazon";
// print_r($query); exit;

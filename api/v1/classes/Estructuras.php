...
class Estructuras
{
    ...
    public function __construct()
    {
        ...
        $this->NameLog = date('Ymd') . '_estructuras.log';
    }
    ...
    public function estructuras()
    {
        ...
        try {
            ...
            if (empty($data)) {
                throw new \Exception('No hay datos', 400);
            }
            ...
            $this->conect->close($conn);
            throw new \Exception('Error al obtener estructuras', 400);
        } catch (\Exception $e) {
            $this->log->trace('Estructuras::' . __FUNCTION__ . ': ', $this->NameLog, $e);
            $this->conect->close($conn);
            throw $e;
    ...
    public function create()
    {
        ...
        try {
            ...
            if ($existe) {
                throw new \Exception('El código (' . $datos['Cod'] . ') ya existe', 400);
            }
            ...
            if ($existe) {
                throw new \Exception('La descripción (' . $datos['Desc'] . ') ya existe', 400);
            }

$(function () {
    ...
    let tablePlantPlanos = $("#tablePlantPlanos").dataTable({ //inicializar datatable
        ...
        responsive: true,
        dom:
            "<'row mt-3 divFilaPlanPlano animate__animated animate__fadeIn'<'col-12 col-sm-6 d-flex justify-content-start d-none d-sm-block'<'divCheck'>><'col-12 col-sm-6 d-flex justify-content-end'<'divAsign'>f>>" +
        ...
        ajax: {
            url: `data/getPlantillaPlanos.php?${Date.now()}`,
            type: "POST",
            dataType: "json",
            data: function (data) {
            ...
            error: function () {
                $("#tablePlantPlanos").css("display", "none");
            }
        },
        ...
        columns: [
            {
                ...
                render: function (data, type, row, meta) {
                    let checked = (row.PlanoSet == true) ? "checked" : "";
                    let datacol =
                        `<label class="form-selectgroup-item flex-fill mb-1">
                        <input ${checked} type="checkbox" name="PlanoID[]" value="${row["PlanoID"]}" class="form-selectgroup-input inputProc">
                        <div class="form-selectgroup-label d-flex align-items-center p-3">
                            ...
                            <div class="form-selectgroup-label-content d-flex align-items-center justify-content-start">
                                <div>
                                    <div class="font-weight-bold">${row["PlanoDesc"]}</div>
                                    <div class="text-muted font08 mt-1">Codigo: ${row["PlanoCod"]}</div>
                                </div>
                            </div>
                        </div>
                    </label>`;
    ...
    function asignPlano(checked) { 
        $.ajax({
            ...
            data: {
                plantillaPlano: true,
                checkPlanos: JSON.stringify(checked),
                PlaPlanoID: $('#selPlantilla').val(),
                PlanPlaDesc: $('#nombrePlantilla').text(),
            },
            ...
            success: function (data) {
                ...
                $("#tablePlantillas").on("draw.dt", function (e, settings) {
                    ...
                    $(`#tablePlantillas .cardBody${$('#selPlantilla').val()} .asignPlano`).removeClass('btn-outline-tabler')
                    $(`#tablePlantillas .cardBody${$('#selPlantilla').val()} .asignPlano`).addClass('btn-tabler')
                });
            },
        });
    } //fin bindForm

    tablePlantPlanos.on("init.dt", function (e, settings) { // Cuando se inicializa la tabla
    ...
    tablePlantPlanos.on("draw.dt", function (e, settings) {  // Se agrega el evento draw.dt para que se 
        ...
        $("#tablePlantPlanos thead").remove(); // Se remueve el thead de la tabla
        ...
        if (settings.json.objPlanoLength != procActivos.length) {
            $.ajax({
                url: "op/crud.php",
                type: "POST",
                data: {
                    plantillaProc: true,
                    checkPlanos: JSON.stringify(procActivos),
                    PlaPlanoID: $('#selPlantilla').val(),
                    PlanPlaDesc: $('#nombrePlantilla').text(),
                    actualizar: 'true',
                },
